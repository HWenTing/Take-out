...
public class deliver_balance_Activity extends AppCompatActivity {
    ...
    private void reflash(){
        ...
        Gson gson = new Gson();
        Deliver shop = gson.fromJson(response,Deliver.class).getDeliver();
        double former = shop.getDeliver_balance();
    ...
    private void showMoneyInDialog() {
        ...
        //显示对话框
        AlertDialog.Builder builder = new AlertDialog.Builder(deliver_balance_Activity.this);
        final AlertDialog dialog = builder.create();
        View view = View.inflate(deliver_balance_Activity.this, R.layout.change_money, null);
        ...
        confirm.setOnClickListener(new View.OnClickListener() {//
            ...
            public void onClick(View v) {
                ...
                Request request = new Request.Builder()
                        .url("http://10.0.2.2:8080/DBDesign/db/deliver_in_money.action")
                        .post(requestBody)
                        .build();
                String response = MyRequest.sendRequestWithOkHttp(request);
                if(response.equals("success")){
                    Toast.makeText(deliver_balance_Activity.this,"充入成功",Toast.LENGTH_SHORT).show();
                    reflash();
                }else{
                    Toast.makeText(deliver_balance_Activity.this,"充入失败",Toast.LENGTH_SHORT).show();
                }
                dialog.dismiss();
            }
        });
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                RequestBody requestBody = new FormBody.Builder()
                        ...
                        .build();
                Request request = new Request.Builder()
                        .url("http://10.0.2.2:8080/DBDesign/db/deliver_out_money.action")
                        .post(requestBody)
                        .build();
                String response = MyRequest.sendRequestWithOkHttp(request);
                if(response.equals("success")){
                    Toast.makeText(deliver_balance_Activity.this,"转出成功",Toast.LENGTH_SHORT).show();
                    reflash();
                }else{
                    Toast.makeText(deliver_balance_Activity.this,"转出失败",Toast.LENGTH_SHORT).show();
                }
                dialog.dismiss();
            }
        });
