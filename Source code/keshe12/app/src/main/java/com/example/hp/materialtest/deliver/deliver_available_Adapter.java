...
public class deliver_available_Adapter extends RecyclerView.Adapter<deliver_available_Adapter.ViewHolder>{
    ...
    public deliver_available_Adapter(List<deliver_order_Item> orderList){
        mOrderList=orderList;
    }

    @Override
    public deliver_available_Adapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (mcontext == null) {
            mcontext = parent.getContext();
        }
        View view= LayoutInflater.from(mcontext).inflate(R.layout.deliver_order_item,parent,false);
        final deliver_available_Adapter.ViewHolder holder = new deliver_available_Adapter.ViewHolder(view);
        ...
        holder.orderView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                RequestBody requestBody = new FormBody.Builder()
                        ...
                        .add("deliver_name",deliver_available_order_Activity.input_account)
                        .build();

                Request request = new Request.Builder()
                        .url("http://10.0.2.2:8080/DBDesign/db/receiveOrder.action")
                        .post(requestBody)
                        .build();

                String response = MyRequest.sendRequestWithOkHttp(request);
                if(response.equals("success")){
                    Toast.makeText(DeliverMainActivity.activity,"接单成功",Toast.LENGTH_SHORT).show();
                }else {
                    Toast.makeText(DeliverMainActivity.activity,"接单失败",Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public void onBindViewHolder(deliver_available_Adapter.ViewHolder holder, int position) {
